    ...
    void Update() {
        ...
        if (player.LockedOn) {
            ...
            if (Enemies.Count > 0 && EnemyTarget != null)
                player.AimmingPoint.transform.position = EnemyTarget.transform.position + new Vector3(0, 1.5f, 0);
            else {
                player.AimmingPoint.transform.position = transform.position + new Vector3(0, 1.5f, 10);
            }
        }
        if (player.LockedOn)
            GetInput();
